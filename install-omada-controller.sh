...
#supported       :Ubuntu 20.04 (focal), 22.04 (jammy), 24.04 (noble), 24.10 (oracular*), 25.04 (plucky*), Debian 12 (bookworm)
#author          :xor0x1
#updated         :2026-06-16
...
case "${VERSION_CODENAME:-}" in
  ...
  bookworm) OS_CODENAME="$VERSION_CODENAME" ;;
  *) die "Поддерживаются Ubuntu 20.04/22.04/24.04/24.10/25.04 и Debian 12 (bookworm)";;
esac
ARCH="$(dpkg --print-architecture)"  # amd64|arm64
info "Обнаружена ${PRETTY_NAME:-$ID $VERSION_ID} ($OS_CODENAME), arch=$ARCH"
...
if [[ "$OMADA_INSTALLED" -eq 0 ]]; then
  ...
  MONGO_REPO_COMP="multiverse"
  if [[ "$ID" == "debian" ]]; then
    MONGO_REPO_DISTRO="debian"
    MONGO_REPO_COMP="main"
  elif [[ "$OS_CODENAME" == "oracular" || "$OS_CODENAME" == "plucky" ]]; then
  ...
  echo "deb [arch=amd64,arm64 signed-by=/usr/share/keyrings/mongodb-server-8.0.gpg] https://repo.mongodb.org/apt/${MONGO_REPO_DISTRO} ${MONGO_REPO_CODENAME}/mongodb-org/8.0 ${MONGO_REPO_COMP}" \
...
PREF
  ...
  # Удаляем старые пакеты MongoDB из репо дистрибутива (конфликтуют с официальными)
  ...
  # Java: Debian 12 в основном репо имеет openjdk-17, Ubuntu — openjdk-21
  if [[ "$ID" == "debian" ]]; then
    JRE_PKG="openjdk-17-jre-headless"
  else
    JRE_PKG="openjdk-21-jre-headless"
  fi
  apt-get install -y -o Dpkg::Options::="--force-overwrite" mongodb-org "$JRE_PKG" jsvc
